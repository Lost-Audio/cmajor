...
| `include/cmajor/helpers/cmaj_JUCEPlugin.h`, `tests/feather/rolling_capture_check.py` | Loader-level rolling recorder v1. Dynamic `CmajPlugin` allocates a fixed 30-second output ring at `prepareToPlay`, captures post-channel-adaptation main output in `processBlock`, snapshots off the audio thread, and writes timestamped float32 WAVs to `%USERPROFILE%\Documents\LostAudio\Captures` from the native loader panel/test path. | Preserve loader-only scope and RT contract: no audio-thread allocation, locks, or file I/O; generated plugins stay unchanged. Validate with `CmajPlugin_VST3`, `tests/feather/rolling_capture_check.py`, the loader state probe, sidechain/mono smokes, and pluginval strictness 8 when `pluginval.exe` is available. |
| `include/cmajor/helpers/cmaj_Patch.h`, `include/cmajor/helpers/cmaj_JUCEPlugin.h`, `javascript/cmaj_api/cmaj-patch-connection.js`, `javascript/cmaj_api/cmaj-generic-patch-view.js`, `tools/command/Source/cmaj_command_CreatePatch.h`, `tests/feather/preset_roundtrip_check.py` | Lost Audio preset system v1. Native JUCE wrappers store user `.lapreset` JSON files at `Documents/LostAudio/<productId>/presets`, keyed by loaded patch ID, reusing `Patch::getFullStoredState()`/`setFullStoredState()` for state capture/apply. JS patch views get list/save/load/delete request APIs; the Svelte scaffold includes a drop-in preset browser and generic views expose compact preset controls. | Preserve wrapper-owned file I/O and keep `Patch` as the state primitive owner. Generated plugins and the dynamic loader both use the loaded patch product ID; the bare loader exposes presets only after a patch is loaded. Regenerate embedded helpers after edits and validate with `tests/feather/preset_roundtrip_check.py`, existing Feather smokes, clean `cmaj`/`CmajPlugin_VST3` builds, and pluginval strictness 8. Factory presets should ship later either embedded in generated resources or installed beside product preset folders, then copied/indexed without changing user presets. |
